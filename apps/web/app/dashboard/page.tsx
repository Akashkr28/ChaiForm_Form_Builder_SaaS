                ...
                <div className="mt-4 space-y-3">
                  ...
                  {draftFields.map((field, index) => (
                    <div key={`${field.id}-${index}`} className="rounded-md border border-black/10 p-2">
                      <input value={field.label} onChange={(event) => updateDraftField(index, { label: event.target.value, id: event.target.value.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/(^_|_$)/g, "") || field.id })} className="w-full rounded-md border border-black/10 px-2 py-1 text-xs" />
